...
export function useDashboardCTO() {
  ...
  const proximasEntregas = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('projetos')
        .select('id, titulo, cliente, data_entrega, progresso, prioridade')
        .gte('data_entrega', today!)
        .lte('data_entrega', nextWeek!)
  ...
  const proximosMilestones = useQuery({
    queryKey: ['dashboard-proximos-milestones'],
    queryFn: async () => {
      const today = new Date().toISOString().split('T')[0]
  ...
  const entreguesMes = allProjetos.filter((p) => {
    if (p.status !== 'entregue') return false
    const d = new Date(p.updated_at)
    return d.getMonth() === currentMonth && d.getFullYear() === currentYear
  })
  const today = new Date().toISOString().split('T')[0]!
  const atrasados = allProjetos.filter((p) => {
    const entrega = getProjetoEntrega(p)
    return entrega && entrega < today && !['entregue', 'cancelado'].includes(p.status)
  })
  ...
  const statusDistribuicao = [
    ...
    { name: 'Em Andamento', value: allProjetos.filter((p) => p.status === 'em_andamento').length, color: '#00C8F0' },
  ...
  return {
    ...
    statusDistribuicao,
    proximasEntregas: proximasEntregas.data ?? [],
    proximosMilestones: proximosMilestones.data ?? [],
    allProjetos,
  }
}
