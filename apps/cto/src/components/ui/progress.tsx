...
function getProgressGradient(value: number): string {
  ...
  if (value < 75) return 'linear-gradient(90deg, #F59E0B, #00C8F0)'
    ...
    <div className={cn('flex items-center gap-2', className)}>
      <div className="progress-bar flex-1">
        <div
          ...
          style={{ width: `${clampedValue}%`, background: getProgressGradient(clampedValue) }}
        />
      </div>
      {showLabel && (
        <span className="text-xs font-medium text-text-secondary w-9 text-right">
